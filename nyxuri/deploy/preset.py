...
The active choice lives in a state file ``~/.config/nyxuri/presets/<app>.active``
...
def _is_deployable_app(app: str) -> bool:
    if not _is_safe_component(app):
        return False
    from nyxuri.deploy.manifest import discover_deployable_apps
...
def _open_presets_dir(*, create: bool = False) -> int:
    ...
    try:
        nyx_fd = _open_child_dir(config_fd, STORAGE_NAME, create=create)
    finally:
        os.close(config_fd)
    try:
...
def resolve_preset_inheritance(
    ...
    try:
        from nyxuri.deploy.manifest import load_manifest_for
...
def get_preset_info(app: str, name: str) -> PresetInfo:
    ...
    if not valid:
        ...
        if src == official:
            ...
            is_editable = True
            is_deletable = True
            rel_path = f"~/.config/{STORAGE_NAME}/presets/{app}/{name}"
    else:
...
def edit_preset(app: str, name: str) -> bool:
    ...
    presets under ~/.config/nyxuri/presets/<app>/<name>/ are editable in place;
