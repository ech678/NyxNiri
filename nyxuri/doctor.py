...
from nyxuri.core import get_env, get_pics_dir, log_msg, timed_run
...
def _check_noctalia(env) -> None:
    from nyxuri.state.ledger import active_shell
...
def _check_fcitx_skin(env) -> None:
    if shutil.which("fcitx5") or (env.config_dir / "fcitx5" / "conf" / "classicui.conf").is_file():
        from nyxuri.modules.fcitx import fcitx_enabled
        if fcitx_enabled():
...
def _check_gtk_theme(env) -> None:
    from nyxuri.modules.gtktheme import gtktheme_registered, gtktheme_rendered
    if gtktheme_rendered():
        ...
        print(msg("doctor_warn", text(
            "GTK 主题: 模板已注册但未渲染，运行 nyxuri gtk install",
            "GTK theme: registered but not rendered, run nyxuri gtk install",
        )))
    else:
...
def _check_preset_drift(env) -> None:
    ...
    """
    from nyxuri.deploy import discover_config_items
    from nyxuri.deploy import InvalidActivePresetError, read_active_preset
    from nyxuri.deploy.preset import _find_preset_src
...
def generate_bug_report() -> Optional[Path]:
    ...
    report_file = env.state_dir / f"nyxuri-bug-report-{timestamp}.md"
    ...
    report = (
        f"# {PROJECT_NAME} Diagnostic Bug Report\n\n"
        f"- **Generated At**: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"- **{PROJECT_NAME} Version**: {env.version}\n"
        ...
        f"```text\n{daemon_status}\n```\n\n"
        f"## 6. {PROJECT_NAME} Health Checks\n\n"
        f"```text\n{health_checks}\n```\n\n"
        f"## 7. Noctalia Hook Log (Last 20 Lines)\n\n"
        f"```text\n{hook_log}\n```\n\n"
        f"## 8. Systemd User Journal (Last 30 Lines)\n\n"
        f"```text\n{journal}\n```\n\n"
        f"## 9. {PROJECT_NAME} Installer Log (Last 30 Lines)\n\n"
        f"```text\n{recent_log}\n```\n"
    )
