...
from nyxuri.core import log_msg, timed_run
...
def greeter_install() -> bool:
    ...
    polkit_rule = (
        ...
        f'    if (action.id == "org.{THEME_ENGINE}.greeter.sync-appearance" &&\n'
...
def greeter_uninstall() -> bool:
    ...
    bak = Path(f"{GREETER_ETC_CFG}.nyxuri.bak")
    if bak.is_file():
