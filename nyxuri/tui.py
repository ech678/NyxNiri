...
def show_logo(env: Optional[Environment] = None) -> int:
    """Render the official Nyxuri ASCII brand header.
    ...
    logo = (
        ...
        " ╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝     ╚═════╝ ╚═╝  ╚═╝╚═╝\n"
        f"{Colors.RESET}\n"
        f"  {Colors.BOLD_CYAN}Nyxuri Desktop Environment Setup{Colors.RESET} {Colors.DARK_GRAY}|{Colors.RESET} {Colors.BOLD_WHITE}{env.version}{Colors.RESET}\n"
        f"  {Colors.DARK_GRAY}{mode_line}{Colors.RESET}\n\n"
    )
    write_cleared(logo)
    return logo.count("\n")
...
def prompt_confirm(prompt_key: str, default: str = "y", destructive: bool = False) -> bool:
    ...
    even under NYXURI_AUTO_YES: express mode speeds things up, it must not
    consent to data loss on the user's behalf.
    """
    if get_compat_env("AUTO_YES", "0") == "1" and not destructive:
        return True
...
def select_language() -> str:
    ...
    if not sys.stdin.isatty():
        from nyxuri.i18n import get_lang
        return get_lang()

    from nyxuri.i18n import set_lang
    env = get_env()
